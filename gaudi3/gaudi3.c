...
/* TODO: need to track SW-71925 and see if this value can be modified */
...
void gaudi3_iterate_nics(struct hl_device *hdev, struct iterate_module_ctx *ctx)
{
	...
	for (die = 0 ; die < prop->num_of_dies ; die++) {
		for (inst = 0 ; inst < NIC_NUM_MACROS_PER_DIE ; inst++) {
			...
			nic_macro = &hdev->nic.nic_macros[nic_id];
		...
		u32 blk_idx, u32 major, u32 minor, u32 sub_minor)
{
	...
	case GAUDI3_BLOCK_TYPE_NIC:
		...
		nic_macro = &hdev->nic.nic_macros[instance_idx];
...
int gaudi3_special_blocks_config(struct hl_device *hdev)
{
	...
	if (ARRAY_SIZE(pb_skip_block_types)) {
		prop->skip_pb_blocks_cfg.block_types =
				kmalloc_array(ARRAY_SIZE(pb_skip_block_types),
					sizeof(pb_skip_block_types[0]), GFP_KERNEL);
		if (!prop->skip_pb_blocks_cfg.block_types) {
			rc = -ENOMEM;
			goto free_special_blocks;
		}

		memcpy(prop->skip_pb_blocks_cfg.block_types, pb_skip_block_types,
					sizeof(pb_skip_block_types));

		prop->skip_pb_blocks_cfg.block_types_len = ARRAY_SIZE(pb_skip_block_types);
	}

	if (ARRAY_SIZE(pb_skip_block_ranges)) {
		prop->skip_pb_blocks_cfg.block_ranges =
				kmalloc_array(ARRAY_SIZE(pb_skip_block_ranges),
					sizeof(pb_skip_block_ranges[0]), GFP_KERNEL);
		if (!prop->skip_pb_blocks_cfg.block_ranges) {
			rc = -ENOMEM;
			goto free_block_types;
		}

		for (i = 0 ; i < ARRAY_SIZE(pb_skip_block_ranges) ; i++)
			memcpy(&prop->skip_pb_blocks_cfg.block_ranges[i],
					&pb_skip_block_ranges[i], sizeof(struct range));

		prop->skip_pb_blocks_cfg.block_ranges_len = ARRAY_SIZE(pb_skip_block_ranges);
	}
...
static void gaudi3_init_sedma_channels(struct hl_device *hdev, u32 reg_base)
{
...
#endif
	err_enable = FIELD_PREP(HD1_SEDMA0_CH0_ERR_ENABLE_EN_MSG_ENG_HBW_RD_RSP_ERR_M, 1) |
			FIELD_PREP(HD1_SEDMA0_CH0_ERR_ENABLE_EN_STL_ENG_HBW_RD_RSP_ERR_M, 1) |
			FIELD_PREP(HD1_SEDMA0_CH0_ERR_ENABLE_EN_MSG_ENGINE_TIMEOUT_M, 1) |
			FIELD_PREP(HD1_SEDMA0_CH0_ERR_ENABLE_EN_STALL_ENGINE_TIMEOUT_M, 1) |
			FIELD_PREP(HD1_SEDMA0_CH0_ERR_ENABLE_EN_MSG_ENG_LBW_RD_RSP_ERR_M, 1) |
			FIELD_PREP(HD1_SEDMA0_CH0_ERR_ENABLE_EN_STL_ENG_LBW_RD_RSP_ERR_M, 1) |
			FIELD_PREP(HD1_SEDMA0_CH0_ERR_ENABLE_EN_MSG_ENG_HBW_WR_RSP_ERR_M, 1) |
			FIELD_PREP(HD1_SEDMA0_CH0_ERR_ENABLE_EN_STL_ENG_HBW_WR_RSP_ERR_M, 1) |
			FIELD_PREP(HD1_SEDMA0_CH0_ERR_ENABLE_EN_MSG_ENG_LBW_WR_RSP_ERR_M, 1) |
			FIELD_PREP(HD1_SEDMA0_CH0_ERR_ENABLE_EN_STL_ENG_LBW_WR_RSP_ERR_M, 1) |
			FIELD_PREP(HD1_SEDMA0_CH0_ERR_ENABLE_EN_MSG_ENG_MSG_WR_ERR_M, 1) |
			FIELD_PREP(HD1_SEDMA0_CH0_ERR_ENABLE_EN_STL_ENG_MSG_WR_ERR_M, 1) |
			FIELD_PREP(HD1_SEDMA0_CH0_ERR_ENABLE_EN_MSG_ENG_NAN_DETECT_M, 0) |
			FIELD_PREP(HD1_SEDMA0_CH0_ERR_ENABLE_EN_STL_ENG_NAN_DETECT_M, 0) |
			FIELD_PREP(HD1_SEDMA0_CH0_ERR_ENABLE_EN_MSG_ENG_INF_DETECT_M, 0) |
			FIELD_PREP(HD1_SEDMA0_CH0_ERR_ENABLE_EN_STL_ENG_INF_DETECT_M, 0) |
			FIELD_PREP(HD1_SEDMA0_CH0_ERR_ENABLE_EN_MSG_LBW_CH_NUM_EXCEED_M, 1) |
			FIELD_PREP(HD1_SEDMA0_CH0_ERR_ENABLE_EN_STL_LBW_CH_NUM_EXCEED_M, 1) |
			FIELD_PREP(HD1_SEDMA0_CH0_ERR_ENABLE_EN_MSG_ENG_SRC_RANGE_ERR_M, 1) |
			FIELD_PREP(HD1_SEDMA0_CH0_ERR_ENABLE_EN_STL_ENG_SRC_RANGE_ERR_M, 1) |
			FIELD_PREP(HD1_SEDMA0_CH0_ERR_ENABLE_EN_MSG_ENG_DST_RANGE_ERR_M, 1) |
			FIELD_PREP(HD1_SEDMA0_CH0_ERR_ENABLE_EN_STL_ENG_DST_RANGE_ERR_M, 1) |
			FIELD_PREP(HD1_SEDMA0_CH0_ERR_ENABLE_EN_STL_ENG_BAD_CONFIG_ERR_M, 1);

	axcache = FIELD_PREP(HD1_SEDMA0_CH0_HBW_AXCACHE_RD_M, AXCACHE_DO_NOT_SKIP_CACHE) |
			FIELD_PREP(HD1_SEDMA0_CH0_HBW_AXCACHE_WR_M, AXCACHE_DO_NOT_SKIP_CACHE);
	...
	for (channel = 0 ; channel < NUM_OF_EDMA_CHANNELS ; channel++) {
		...
		WREG32(reg_base + channel_offset + mmEDMA_CHN_PROT_SECURE,
				FIELD_PREP(HD1_SEDMA0_CH0_PROT_SECURE_VAL_M, 0x1));
		...
		/* TODO: enable when F/W events are defined */
#if 0
		WREG32(reg_base + channel_offset + SEDMA_CH_ERR_MSG_ADDR_LO_OFFSET,
				lower_32_bits(CFG_BAR_BASE + irq_handler_offset));
		WREG32(reg_base + channel_offset + SEDMA_CH_ERR_MSG_ADDR_HI_OFFSET,
				upper_32_bits(CFG_BAR_BASE + irq_handler_offset));
		WREG32(reg_base + channel_offset + SEDMA_CH_ERR_MSG_WDATA_OFFSET,
...
{
	u32 reg_val, eu_id, eu_offset;

	reg_val = FIELD_PREP(MME_ACC_INTR_MASK_WBC_ERR_RESP_MASK, 0) |
			FIELD_PREP(MME_ACC_INTR_MASK_WBC_BUSER_SLV_ERR_MASK, 0) |
			FIELD_PREP(MME_ACC_INTR_MASK_WBC_BUSER_NUMERICAL_INF_ERR_MASK, 1) |
			FIELD_PREP(MME_ACC_INTR_MASK_WBC_BUSER_NUMERICAL_NINF_ERR_MASK, 1) |
			FIELD_PREP(MME_ACC_INTR_MASK_WBC_BUSER_NUMERICAL_NAN_ERR_MASK, 1) |
			FIELD_PREP(MME_ACC_INTR_MASK_WBC_BUSER_RR_DBG_ERR_MASK, 1) |
			FIELD_PREP(MME_ACC_INTR_MASK_AP_STS_SRC_DNRM_INTR_P_MASK, 1) |
			FIELD_PREP(MME_ACC_INTR_MASK_AP_STS_SRC_INF_INTR_P_MASK, 1) |
			FIELD_PREP(MME_ACC_INTR_MASK_AP_STS_SRC_NINF_INTR_P_MASK, 1) |
			FIELD_PREP(MME_ACC_INTR_MASK_AP_STS_SRC_NAN_INTR_P_MASK, 1) |
			FIELD_PREP(MME_ACC_INTR_MASK_AP_STS_RES_INF_INTR_P_MASK, 1) |
			FIELD_PREP(MME_ACC_INTR_MASK_AP_STS_RES_NINF_INTR_P_MASK, 1);

	for (eu_id = 0 ; eu_id < NUM_OF_MME_EU_PER_HDCORE ; eu_id++) {
		eu_offset = eu_id * HDCORE_MME_EU_OFFSET;
		WREG32(reg_base + mmACC_INTR_MASK + eu_offset, reg_val);
	}
}
...
int gaudi3_enable_msix(struct hl_device *hdev)
{
	...
	if (hdev->pldm) {
		...
		for (i = GAUDI3_PLDM_IRQ_FIRST, pldm_irq_cnt = 0;
				i <= GAUDI3_PLDM_IRQ_LAST ; i++, pldm_irq_cnt++) {
			irq = hl_irq_vector(hdev, i);

			irq_handler = hl_pldm_irq_handler;

			rc = request_irq(irq, irq_handler, 0, gaudi3_irq_name(i), hdev);
			if (rc) {
				dev_err(hdev->dev, "Failed to request IRQ %d", irq);
				for (i = GAUDI3_PLDM_IRQ_FIRST;
						i < GAUDI3_PLDM_IRQ_FIRST + pldm_irq_cnt;
						i++) {
					irq = hl_irq_vector(hdev, i);
					free_irq(irq, hdev);
				}
...
void gaudi3_user_interrupt_setup(struct hl_device *hdev)
{
	...
	for (i = 0, j = 0, intr_id = GAUDI3_IRQ_NUM_DEC_NRM_FIRST;
			i < prop->user_dec_intr_count;
			i++, j++, intr_id++)
		HL_USR_INTR_STRUCT_INIT(hdev->user_interrupt[j], hdev, intr_id, true);

	/* Initialize user interrupt */
	for (i = 0, intr_id = GAUDI3_IRQ_NUM_USER_FIRST;
			i < prop->user_interrupt_count;
			i++, j++, intr_id++)
		HL_USR_INTR_STRUCT_INIT(hdev->user_interrupt[j], hdev, intr_id, false);
}
...
static bool gaudi3_is_engine_enabled(struct hl_device *hdev, u32 eng_id)
{
	...
	case GAUDI3_DIE0_ENGINE_ID_NIC_0 ... GAUDI3_DIE0_ENGINE_ID_NIC_5:
		...
		nic_macro = &hdev->nic.nic_macros[nic_id];
		return !!(hdev->nic_ports_mask & gaudi3_nic_get_macro_ports_mask(nic_macro));
					...
					struct iterate_module_ctx *ctx)
{
	...
	WREG32(edma_cmn_reg_base + mmEDMA_CMN_CFG,
			FIELD_PREP(HD1_SEDMA0_CMN_CFG_HALT_ALL_M, 0x1));
}
...
static void gaudi3_no_fw_monitor(struct hl_device *hdev, bool *stop_monitor)
{
	...
	/* TODO: delete that call once interrupt aggregator support is merged
	 * also set stop_monitor = true
	 */
...
void gaudi3_handle_eqe(struct hl_device *hdev, struct hl_eq_entry *eq_entry)
{
	...
	case GAUDI3_EVENT_ARC_FARM0_DIE0_HD0_SEI:
	case GAUDI3_EVENT_ARC_FARM0_DIE0_HD1_SEI:
	case GAUDI3_EVENT_ARC_FARM0_DIE0_HD2_SEI:
	case GAUDI3_EVENT_ARC_FARM0_DIE0_HD3_SEI:
	case GAUDI3_EVENT_ARC_FARM0_DIE1_HD0_SEI:
	 ...
	 */
	case GAUDI3_EVENT_NIC0_DIE0_HDSHARED_SPI_0:
	case GAUDI3_EVENT_NIC1_DIE0_HDSHARED_SPI_0:
	case GAUDI3_EVENT_NIC2_DIE0_HDSHARED_SPI_0:
	case GAUDI3_EVENT_NIC3_DIE0_HDSHARED_SPI_0:
	case GAUDI3_EVENT_NIC4_DIE0_HDSHARED_SPI_0:
	case GAUDI3_EVENT_NIC5_DIE0_HDSHARED_SPI_0:
		index = (event_type - GAUDI3_EVENT_NIC0_DIE0_HDSHARED_SPI_0) / 2;
		gaudi3_nic_handle_bmon_spmu_event(hdev, index, &eq_entry->intr_cause, &event_mask);
		break;

	case GAUDI3_EVENT_NIC0_DIE0_HDSHARED_SPI_1:
	case GAUDI3_EVENT_NIC1_DIE0_HDSHARED_SPI_1:
	case GAUDI3_EVENT_NIC2_DIE0_HDSHARED_SPI_1:
	case GAUDI3_EVENT_NIC3_DIE0_HDSHARED_SPI_1:
	case GAUDI3_EVENT_NIC4_DIE0_HDSHARED_SPI_1:
	case GAUDI3_EVENT_NIC5_DIE0_HDSHARED_SPI_1:
		index = (event_type - GAUDI3_EVENT_NIC0_DIE0_HDSHARED_SPI_1) / 2;
		gaudi3_handle_nic_spi_event(hdev, index, &eq_entry->intr_cause);
		break;

	case GAUDI3_EVENT_NIC0_DIE1_HDSHARED_SPI_0:
	case GAUDI3_EVENT_NIC1_DIE1_HDSHARED_SPI_0:
	case GAUDI3_EVENT_NIC2_DIE1_HDSHARED_SPI_0:
	case GAUDI3_EVENT_NIC3_DIE1_HDSHARED_SPI_0:
	case GAUDI3_EVENT_NIC4_DIE1_HDSHARED_SPI_0:
	case GAUDI3_EVENT_NIC5_DIE1_HDSHARED_SPI_0:
		/* Adding number of macros of DIE0 as offset */
		index = (event_type - GAUDI3_EVENT_NIC0_DIE1_HDSHARED_SPI_0) / 2 +
			NIC_NUM_MACROS_PER_DIE;
		gaudi3_nic_handle_bmon_spmu_event(hdev, index, &eq_entry->intr_cause, &event_mask);
		break;

	case GAUDI3_EVENT_NIC0_DIE1_HDSHARED_SPI_1:
	case GAUDI3_EVENT_NIC1_DIE1_HDSHARED_SPI_1:
	case GAUDI3_EVENT_NIC2_DIE1_HDSHARED_SPI_1:
	case GAUDI3_EVENT_NIC3_DIE1_HDSHARED_SPI_1:
	case GAUDI3_EVENT_NIC4_DIE1_HDSHARED_SPI_1:
	case GAUDI3_EVENT_NIC5_DIE1_HDSHARED_SPI_1:
		/* Adding number of macros of DIE0 as offset */
		index = (event_type - GAUDI3_EVENT_NIC0_DIE1_HDSHARED_SPI_1) / 2 +
			NIC_NUM_MACROS_PER_DIE;
		gaudi3_handle_nic_spi_event(hdev, index, &eq_entry->intr_cause);
		break;
