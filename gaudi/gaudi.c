...
int gaudi_set_fixed_properties(struct hl_device *hdev)
{
	...
	nic_prop->swq_base_size = SWQ_BASE_SIZE;
	nic_prop->txs_base_addr = TXS_BASE_ADDR;
...
static int gaudi_nic_clear_mem(struct hl_device *hdev)
{
	...
	return gaudi_memset_device_memory(hdev, nic_prop->nic_drv_addr,
		nic_prop->nic_drv_size, 0);
}
...
static int gaudi_sw_init(struct hl_device *hdev)
{
	...
	if (rc) {
		dev_err(hdev->dev, "Failed to init fetch memory ranges\n");
		goto free_internal_qmans_pq_mem;
	}
...
static void gaudi_init_rate_limiter(struct hl_device *hdev)
{
	...
	dev_info(hdev->dev, "Device used in compatibility mode\n");
	...
	dev_info(hdev->dev, "Rate limiters are configured\n");
...
static int gaudi_hw_init(struct hl_device *hdev)
{
	...
	if (rc) {
		dev_err(hdev->dev, "failed to initialize HBM\n");
		return rc;
	}
...
static void gaudi_update_nic_qmans_state(struct hl_device *hdev)
{
	...
	for (nic_id = 0 ; nic_id < NIC_NUMBER_OF_ENGINES ; nic_id++) {
		 ...
		 */
		if ((gaudi->hw_cap_initialized & nic_mask) &&
				(!(hdev->nic_ports_mask & BIT(nic_id)))) {
