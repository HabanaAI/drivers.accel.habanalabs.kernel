...
static u32 gaudi_hbm_rd_wdr[8] = {
		0x08, 0x09, 0x0C, 0x0D, 0x0A, 0x0B, 0x0E, 0x0F};
...
static void set_aword_dll(struct hl_device *hdev, u32 base, int ch,
					u32 val)
{
	set_dll_val(hdev, base, ch, 0x428, val);
}
...
static void config_mlb_state_bypass(struct hl_device *hdev, int device)
{
	u32 base = GAUDI_HBM_CFG_BASE + device * GAUDI_HBM_CFG_OFFSET;
	int ch;
...
static void config_mlb_state_dword(struct hl_device *hdev, int device)
{
	u32 base = GAUDI_HBM_CFG_BASE + device * GAUDI_HBM_CFG_OFFSET;
	int ch;

	for (ch = 0 ; ch < GAUDI_HBM_CHANNELS ; ch++)
		config_mlb_state(hdev, base, ch, phy_state_dword);
}

static void config_mlb_state_initial(struct hl_device *hdev, int device)
{
	u32 base = GAUDI_HBM_CFG_BASE + device * GAUDI_HBM_CFG_OFFSET;
	int ch;
...
static void config_mlb_state_aword(struct hl_device *hdev, int device)
{
	u32 base = GAUDI_HBM_CFG_BASE + device * GAUDI_HBM_CFG_OFFSET;
	int ch;

	for (ch = 0 ; ch < GAUDI_HBM_CHANNELS ; ch++)
		config_mlb_state(hdev, base, ch, phy_state_aword);
}
...
static void set_mode_reg_by_mlb_wrap(struct hl_device *hdev, int device)
{
	...
	for (ch = 0 ; ch < GAUDI_HBM_CHANNELS ; ch++)
		set_mode_reg_by_mlb(hdev, base, ch,
...
static void config_dram_state_wrap(struct hl_device *hdev, int device)
{
	u32 base = GAUDI_HBM_CFG_BASE + device * GAUDI_HBM_CFG_OFFSET;
	int ch;
...
static void config_mc_state_bist(struct hl_device *hdev, int device)
{
	int ch;
...
static void config_mc_state_normal(struct hl_device *hdev, int device)
{
	int ch;
...
static int mc_bist(struct hl_device *hdev, int device, int ch,
		enum mc_bist_mode bist_mode, int interleave, u32 sdii_cnt)
{
	struct gaudi_device *gaudi = hdev->asic_specific;
	u32 base = GAUDI_HBM_CFG_BASE + device * GAUDI_HBM_CFG_OFFSET;
	u32 burst_cnt = sdii_cnt; /* max is 0x800000 per PC */
	u32 r1_lfsr_ini = 0x5A5A3C3C;
	u32 r2_lfsr_ini = 0x5A5A3C3C;
	u32 r3_lfsr_ini = 0x5A5A3C3C;
	u32 r4_lfsr_ini = 0x5A5A3C3C;
	u32 status, timeout, val;
	bool lfsr_mode = 1;
	int err;
	...
	WREG32(base + ch * 0x1000 + 0x100, lfsr_mode | (interleave << 1) |
						(bist_mode << 2) |
						(gaudi->hbm_ecc_enable  << 3));
	WREG32(base + ch * 0x1000 + 0x104, 0x0); /* start address */
	WREG32(base + ch * 0x1000 + 0x108, burst_cnt);
	WREG32(base + ch * 0x1000 + 0x10C, r1_lfsr_ini);
	WREG32(base + ch * 0x1000 + 0x110, r2_lfsr_ini);
	WREG32(base + ch * 0x1000 + 0x114, r3_lfsr_ini);
	WREG32(base + ch * 0x1000 + 0x118, r4_lfsr_ini);
	WREG32(base + ch * 0x1000 + 0x11C, 3);

	if (bist_mode == mc_bist_mode_overn)
		return 0;
	...
	if (err) {
		dev_err(hdev->dev,
			"test failed for HBM device %d, ch %d, status : %d\n",
			device, ch, status);
		return -EIO;
	}

	val = RREG32_MASK(base + (ch * 0x1000) + 0x11C, 0xFF000000);
	if (val != 3) {
		dev_err(hdev->dev,
			"HBM reg 0x11C wrong val: 0x%x, device %d, ch %d\n",
			val, device, ch);
		return -EIO;
	}
...
static void read_ecc_counters(struct hl_device *hdev, int stam)
{
	int device, ch;
	u32 base, val;
	...
	for (device = 0 ; device < GAUDI_HBM_DEVICES ; device++) {
		...
		for (ch = 0 ; ch < GAUDI_HBM_CHANNELS ; ch++) {
			val = RREG32_MASK(base + ch * 0x1000 + 0x060,
					0xFFFFFC00);
			if (val)
				dev_err(hdev->dev,
					"ECC ERRORS dev=%d, pc=%d: SERR=%d, Cont=%d, DERR=%d\n",
					device, ch*2, (val>>6)&0xFF, val&0x3F,
					(val>>14)&0xFF);

			val = RREG32_MASK(base + ch * 0x1000 + 0x070,
					0xFFFFFC00);
			if (val)
				dev_err(hdev->dev,
					"ECC ERRORS dev=%d, pc=%d: SERR=%d, Cont=%d, DERR=%d\n",
					device, ch*2+1, (val>>6)&0xFF, val&0x3F,
					(val>>14)&0xFF);
		}
	}
}
...
static void dbg_read_mcbist_status(struct hl_device *hdev, int unused)
{
	int device, ch;
	u32 base, val;
	...
	for (device = 0 ; device < GAUDI_HBM_DEVICES ; device++) {
		base = GAUDI_HBM_CFG_BASE + device * GAUDI_HBM_CFG_OFFSET;
		for (ch = 0 ; ch < GAUDI_HBM_CHANNELS ; ch++) {
			val = RREG32_MASK(base + ch * 0x1000 + 0x11C,
						0x0000000F);
			if (val == 0) {
				val = RREG32_MASK(base + ch * 0x1000 + 0x11C,
							0x0F000000);
				if (val != 3)
					dev_err(hdev->dev,
						"MCBIST failed: dev=%d, ch=%d, status=0x%x\n",
						device, ch, val);
			}
		}
	}
...
static int write_read(struct hl_device *hdev, int device)
{
	struct gaudi_device *gaudi = hdev->asic_specific;
	u32 base = GAUDI_HBM_CFG_BASE + device * GAUDI_HBM_CFG_OFFSET;
	int i, ch, rc = 0;
	...
	for (ch = 0 ; ch < GAUDI_HBM_CHANNELS ; ch++) {
		...
		for (i = 0 ; i < gaudi->hbm_loop ; i++) {
			/* msleep(20); */

			rc = mc_bist(hdev, device, ch, mc_bist_mode_once,
					gaudi->hbm_interleave,
					gaudi->hbm_sdii_cnt);
			if (rc) {
				dev_err(hdev->dev,
					"mcbist failed, device: %d, ch: %d, loop: %d\n",
					device, ch, i);
				break;
			}
		}
...
static int write_read_infinite(struct hl_device *hdev, int unused)
{
	struct gaudi_device *gaudi = hdev->asic_specific;
	u32 base;
	int device, ch, rc;
	...
	for (device = 0 ; device < GAUDI_HBM_DEVICES ; device++) {
		...
		for (ch = 0 ; ch < GAUDI_HBM_CHANNELS ; ch++) {
			dev_dbg(hdev->dev, "ch: %d\n", ch);
			config_mlb_state(hdev, base, ch, phy_state_bypass);
			config_mc_state(hdev, device, ch, mc_state_bist);

			/* using max instead of: gaudi->hbm_sdii_cnt */
			rc = mc_bist(hdev, device, ch, mc_bist_mode_once,
					gaudi->hbm_interleave, 0x800000);
			if (rc) {
				dev_err(hdev->dev,
					"mcbist failed, device: %d, ch: %d\n",
					device, ch);
			...
			/* using max instead of: gaudi->hbm_sdii_cnt */
			mc_bist(hdev, device, ch, mc_bist_mode_overn,
				gaudi->hbm_interleave, 0x800000);
		}
	}
...
static void dbg_dump_regs(struct hl_device *hdev, int stam)
{
	u32 base;
	int device, ch, reg;
	...
	for (device = 0 ; device < 1 ; device++) {
		base = GAUDI_HBM_CFG_BASE + device * GAUDI_HBM_CFG_OFFSET;
		for (ch = 7 ; ch < GAUDI_HBM_CHANNELS ; ch++) {
			for (reg = 0 ; reg < 0x1ff ; reg += 16) {
				dev_info(hdev->dev,
					"DEV %d CH %d REG 0x%03x:\t0x%08x\t0x%08x\t0x%08x\t0x%08x\n",
					device, ch, reg,
					RREG32(base + ch * 0x1000 + reg + 0),
					RREG32(base + ch * 0x1000 + reg + 4),
					RREG32(base + ch * 0x1000 + reg + 8),
					RREG32(base + ch * 0x1000 + reg + 12));
			}
		}
	}
}

static void stop_mc_bist(struct hl_device *hdev, int device)
{
	int ch;

	dev_dbg(hdev->dev, "%s, device: %d\n", __func__, device);
...
static void mc_rstn(struct hl_device *hdev, int device)
{
	/* set bits 33-36 (all in high) */
	WREG32(mmPSOC_GLOBAL_CONF_SW_ALL_RST_CFG_L, 0);
	WREG32(mmPSOC_GLOBAL_CONF_SW_ALL_RST_CFG_H, 1 << (device + 1));
	WREG32(mmPSOC_GLOBAL_CONF_SW_ALL_RST, 1);
}
...
static void i1500_aword_misr_config(struct hl_device *hdev, u32 base,
						int ch, int wdr)
{
	RMWREG32(base + GAUDI_HBM_CENTER_CHANNEL * 0x1000 + 0x100, wdr, 0xFF);
	RMWREG32(base + GAUDI_HBM_CENTER_CHANNEL * 0x1000 + 0x20, 7, 0xFF);
	RMWREG32(base + GAUDI_HBM_CENTER_CHANNEL * 0x1000 + 0x24,
			(ch << 8) | 0xA, 0xFFFF);
	RMWREG32(base + GAUDI_HBM_CENTER_CHANNEL * 0x1000 + 0x28, 5, 0xFF);
}

static int i1500_aword_misr(struct hl_device *hdev, u32 base, int ch)
{
	RMWREG32(base + GAUDI_HBM_CENTER_CHANNEL * 0x1000 + 0x20, 29, 0xFF);
	RMWREG32(base + GAUDI_HBM_CENTER_CHANNEL * 0x1000 + 0x24,
			(ch << 8) | 0xA, 0xFFFF);
	RMWREG32(base + GAUDI_HBM_CENTER_CHANNEL * 0x1000 + 0x28, 6, 0xFF);

	if (RREG32(base + GAUDI_HBM_CENTER_CHANNEL * 0x1000 +
			(gaudi_hbm_rd_wdr[ch] << 8)) == 0x119C712B)
		return 0;
	else
		return -EIO;
}

static void aword_training_by_1500(struct hl_device *hdev, u32 base,
						int ch)
{
	struct gaudi_device *gaudi = hdev->asic_specific;
	u32 pass_left_t = 0, max_pass_left_t = 0, pass_cnt = 0,
		max_pass_cnt = 0, window, right_t, middle_t, n;
	int aw_t;

	dev_dbg(hdev->dev, "%s\n", __func__);

	WREG32(base + ch * 0x1000 + 0x220, 0x20004000);
	WREG32(base + ch * 0x1000 + 0x224, 0x8001000);
	WREG32(base + ch * 0x1000 + 0x228, 0x2000400);
	WREG32(base + ch * 0x1000 + 0x22C, 0x800100);
	WREG32(base + ch * 0x1000 + 0x230, 0x200040);
	WREG32(base + ch * 0x1000 + 0x234, 0);
	WREG32(base + ch * 0x1000 + 0x238, 0x20001);
	WREG32(base + ch * 0x1000 + 0x23C, 0x80004);

	config_mlb_state(hdev, base, ch, phy_state_aword);

	RMWREG32(base + ch * 0x1000 + 0x2FF, 2, 0xFF);
	RMWREG32(base + ch * 0x1000 + 0x244, 1, 0xFF);

	n = aword_acc_window(hdev, base, ch) + 0x20;
	for (aw_t = 0 ; aw_t < n ; aw_t += gaudi->hbm_inc) {
		set_aword_dll(hdev, base, ch, aw_t);
		i1500_aword_misr_config(hdev, base, ch, 8);
		i1500_aword_misr_config(hdev, base, ch, 0xB);

		RMWREG32(base + ch * 0x1000 + 0x240, 1, 0xFF);

		if (!i1500_aword_misr(hdev, base, ch)) {
			if (pass_cnt == 0)
				pass_left_t = aw_t;
			pass_cnt++;
		} else {
	...
	window = max_pass_cnt * gaudi->hbm_inc;
	right_t = max_pass_left_t + window;
	middle_t = max_pass_left_t + (window >> 1);

	set_aword_dll(hdev, base, ch, middle_t);

	dev_dbg(hdev->dev, "interval: 0x%x ~ 0x%x\n", max_pass_left_t,
			right_t);
	dev_dbg(hdev->dev, "window: %d\n", (window + 1) / (middle_t * 100));
	dev_dbg(hdev->dev, "result: 0x%x\n", middle_t);

	i1500_aword_misr_config(hdev, base, ch, 0);
}

/* dir can be 0 - write, 1 - read */
static int i1500_dword_misr(struct hl_device *hdev, u32 base, int ch,
					int ps, int dir)
{
	u32 rd1, rd2;

	if (dir == 0) {
		WREG32(base + ch * 0x1000 + 0x100, 0x1063286F);
		WREG32(base + ch * 0x1000 + 0x104, 0x554925EE);
		WREG32(base + ch * 0x1000 + 0x108, 0x70B81EB5);
		WREG32(base + ch * 0x1000 + 0x10C, 0x2E5957F3);
		WREG32(base + ch * 0x1000 + 0x110, 0x7A350936);
		WREG32(base + ch * 0x1000 + 0x114, 0x35084E63);
		WREG32(base + ch * 0x1000 + 0x118, 0x37E83104);
		WREG32(base + ch * 0x1000 + 0x11C, 0x774157AE);
		WREG32(base + ch * 0x1000 + 0x120, 0x12345678);
		WREG32(base + ch * 0x1000 + 0x124, 0x87654321);
	}

	RMWREG32(base + ch * 0x1000 + 0x20, 319, 0xFF);
	RMWREG32(base + ch * 0x1000 + 0x24, (ch << 8) | 9, 0xFFFF);

	if (dir == 0) {
		RMWREG32(base + ch * 0x1000 + 0x28, 6, 0xFF);
	} else {
		u32 addr = base + ch * 0x1000 +
					(gaudi_hbm_rd_wdr[ch] << 8) + ps * 8;
		rd1 = RREG32(addr);
		rd2 = RREG32(addr + 4);

		if (rd1 == 0xADC9CADC || rd2 == 0xC9CADC9C)
			return 0;
		else
			return -EIO;
	}
...
static void dword_write_training_by_1500(struct hl_device *hdev,
							u32 base, int ch)
{
	struct gaudi_device *gaudi = hdev->asic_specific;
	u32 pass_left_t = 0, max_pass_left_t = 0, pass_cnt = 0,
		max_pass_cnt = 0, right_t = 0, window, middle_t;
	int wr_t, dw;

	dev_dbg(hdev->dev, "%s\n", __func__);

	WREG32(base + ch * 0x1000 + 0x260, 0x1063286F);
	WREG32(base + ch * 0x1000 + 0x264, 0x554925EE);
	WREG32(base + ch * 0x1000 + 0x268, 0x70B81EB5);
	WREG32(base + ch * 0x1000 + 0x26C, 0x2E5957F3);
	WREG32(base + ch * 0x1000 + 0x270, 0x7A350936);
	WREG32(base + ch * 0x1000 + 0x274, 0x35084E63);
	WREG32(base + ch * 0x1000 + 0x278, 0x37E83104);
	WREG32(base + ch * 0x1000 + 0x27C, 0x774157AE);

	WREG32(base + ch * 0x1000 + 0x258, 2);

	for (wr_t = 0 ; wr_t < (acc[ch][1] + 0x20) ; wr_t += gaudi->hbm_inc) {
		for (dw = 0 ; dw < 4 ; dw++)
			set_dword_write_dll(hdev, base, ch, dw, wr_t);

		config_mlb_state(hdev, base, ch, phy_state_initial);

		set_mode_reg_by_mlb(hdev, base, ch, 7, 3);
		set_mode_reg_by_mlb(hdev, base, ch, 7, 0x1B);

		config_mlb_state(hdev, base, ch, 1);
		RMWREG32(base + ch * 0x1000 + 0x250, 1, 0xFF);

		if (!i1500_dword_misr(hdev, base, ch, 0, 1)) {
			if (pass_cnt == 0)
				pass_left_t = wr_t;
			pass_cnt++;
		} else {
	...
	dev_dbg(hdev->dev, "interval 0x%x ~ 0x%x\n", max_pass_left_t, right_t);
	dev_dbg(hdev->dev, "ratio %d\n", window / (acc[ch][1] * 100));
	dev_dbg(hdev->dev, "result 0x%x\n", middle_t);

	for (dw = 0 ; dw < 4 ; dw++)
		set_dword_write_dll(hdev, base, ch, dw, middle_t);

	config_mlb_state(hdev, base, ch, 2);
	set_mode_reg_by_mlb(hdev, base, ch, 7, 2);
}

static void dword_read_training_by_1500(struct hl_device *hdev, u32 base,
					int ch)
{
	struct gaudi_device *gaudi = hdev->asic_specific;
	u32 pass_left_t = 0, max_pass_left_t = 0, pass_cnt = 0,
		max_pass_cnt = 0, right_t = 0, window, middle_t, val;
	int ps, rd_t, dw;

	dev_dbg(hdev->dev, "%s\n", __func__);

	for (ps = 0 ; ps < 1 ; ps++) {
		val = ps + (gaudi->hbm_mode == hbm_mode_lfsr ? 0 : 8);
		RMWREG32(base + ch * 0x1000 + 0x258, val, 0xFF);

		for (rd_t = 0 ; rd_t < (acc[ch][1] + 20);
				rd_t += gaudi->hbm_inc) {
			for (dw = 0 ; dw < 4 ; dw++)
				set_dword_write_dll(hdev, base, ch,
								dw, rd_t);

			config_mlb_state(hdev, base, ch, phy_state_initial);
			set_mode_reg_by_mlb(hdev, base, ch, 7, 3);
			val = gaudi->hbm_mode == hbm_mode_lfsr ? 0xB : 0x13;
			set_mode_reg_by_mlb(hdev, base, ch, 7, val);

			config_mlb_state(hdev, base, ch, phy_state_dword);
			RMWREG32(base + ch * 0x1000 + 0x254, 1, 0xFF);

			if ((RREG32(base + ch * 0x1000 + 0x514) & 0xFF) == 0) {
				if (pass_cnt == 0)
					pass_left_t = rd_t;
				pass_cnt++;
			} else {
...
static void software_training_by_1500(struct hl_device *hdev,
						int device)
{
	u32 base = GAUDI_HBM_CFG_BASE + device * GAUDI_HBM_CFG_OFFSET;
	int ch;
	...
	for (ch = 0 ; ch < GAUDI_HBM_CHANNELS ; ch++)
		aword_training_by_1500(hdev, base, ch);

	for (ch = 0 ; ch < GAUDI_HBM_CHANNELS ; ch++)
		config_dram_state(hdev, base, ch, dram_state_training);

	for (ch = 0 ; ch < GAUDI_HBM_CHANNELS ; ch++)
		dword_write_training_by_1500(hdev, base, ch);

	for (ch = 0 ; ch < GAUDI_HBM_CHANNELS ; ch++)
		dword_read_training_by_1500(hdev, base, ch);

	restore_pll(hdev);
}

static void dword_write_training_by_bist(struct hl_device *hdev,
							u32 base, int ch)
{
	struct gaudi_device *gaudi = hdev->asic_specific;
	u32 pass_left_t = 0, max_pass_left_t = 0, pass_cnt = 0,
		max_pass_cnt = 0, right_t = 0, window, middle_t, addr;
	int wr_t, dw;

	dev_dbg(hdev->dev, "%s\n", __func__);

	for (dw = 0 ; dw < 4 ; dw++)
		set_dword_write_dll(hdev, base, ch, dw, 0);

	for (wr_t = 0 ; wr_t < (acc[ch][1] + 0x20) ; wr_t += gaudi->hbm_inc) {
		/* MCBIST */
		if (!mc_bist(hdev, base, ch, mc_bist_mode_once,
				gaudi->hbm_interleave, gaudi->hbm_sdii_cnt)) {
		...
		/* Increment DLL value */
		for (dw = 0 ; dw < 4 ; dw++) {
			addr = base + ch * 0x1000 + dw * 0x100 + 0x528;
			if (gaudi->hbm_inc == 1)
				WREG32(addr, 1);
			else if (gaudi->hbm_inc == 4)
				WREG32(addr, 2);
			else if (gaudi->hbm_inc == 16)
				WREG32(addr, 3);
		}
...
static void dword_read_training_by_bist(struct hl_device *hdev, u32 base,
				int ch, int dw, enum bist_trigger trigger)
{
	struct gaudi_device *gaudi = hdev->asic_specific;
	u32 pass_left_t = 0, max_pass_left_t = 0, pass_cnt = 0,
		max_pass_cnt = 0, right_t = 0, range, window, middle_t, addr;
	int rd_t;

	dev_dbg(hdev->dev, "%s\n", __func__);

	range = dword_acc_window(hdev, base, ch, dw);

	if (!range) {
		dev_err(hdev->dev,
			"dword acc_window for ch %d, ch %d can't be 0\n",
			ch, dw);
		return;
	}
	...
	for (rd_t = 0 ; rd_t < range ; rd_t += gaudi->hbm_inc) {
		/* MCBIST */
		if (!mc_bist(hdev, base, ch, mc_bist_mode_once,
				gaudi->hbm_interleave, gaudi->hbm_sdii_cnt)) {
		...
		/* Increment DLL value */
		switch (trigger) {
		case bist_trigger_falling:
			dev_err(hdev->dev, "bist trigger %d not implemented\n",
				trigger);
			return;
		case bist_trigger_rising:
			dev_err(hdev->dev, "bist trigger %d not implemented\n",
				trigger);
			return;
		case bist_trigger_edge:
			addr = base + ch * 0x1000 + dw * 0x100 + 0x518;
			if (gaudi->hbm_inc == 1) {
				/* 0x518 - rising */
				WREG32(addr,   1);
				/* 0x51C - falling */
			...
			break;
		default:
			dev_err(hdev->dev, "unknown bist trigger %d\n",
				trigger);
			return;
		}
...
static void software_training_by_bist(struct hl_device *hdev, int device)
{
	struct gaudi_device *gaudi = hdev->asic_specific;
	u32 base = GAUDI_HBM_CFG_BASE + device * GAUDI_HBM_CFG_OFFSET;
	int ch, dw, rc;
	...
	for (ch = 0 ; ch < GAUDI_HBM_CHANNELS ; ch++) {
		...
		config_mlb_state(hdev, base, ch, phy_state_aword);
		set_aword_dll(hdev, base, ch, gaudi->hbm_static_dll_aword);

		for (dw = 0 ; dw < 4 ; dw++)
			set_dword_write_dll(hdev, base, ch, dw,
						gaudi->hbm_static_dll_dword_wr);

		for (dw = 0 ; dw < 4 ; dw++)
			set_dword_read_dll(hdev, base, ch, dw,
	...
	rc = write_read(hdev, device);
	if (rc) {
		dev_err(hdev->dev,
			"Static value for DWORD write or read failed a write-read operation\n");
		dev_err(hdev->dev,
			"there's no basis for training using MCBIST\n");
		dev_err(hdev->dev,
			"Either try to change the static values, or use IEEE1500 training instead\n");
		goto out;
	}
	...
	for (ch = 0 ; ch < GAUDI_HBM_CHANNELS ; ch++) {
		/* Write */
		dword_write_training_by_bist(hdev, base, ch);

		/* Read */
		for (dw = 0 ; dw < 4 ; dw++)
			dword_read_training_by_bist(hdev, base, ch, dw,
							bist_trigger_edge);
...
/*
 * set_static_params is a manual alternative to Training - it sets approximated
 * values which may be feasible for system operation
 */
static void set_static_params(struct hl_device *hdev, int device)
{
	u32 base = GAUDI_HBM_CFG_BASE + device * GAUDI_HBM_CFG_OFFSET;
	int ch, dw;
	...
	for (ch = 0 ; ch < GAUDI_HBM_CHANNELS ; ch++) {
		/* AWORD master */
		set_dll_val(hdev, base, ch, 0x414, STATIC_DLL_ACC);
		 /* AWORD slave4 */
		set_dll_val(hdev, base, ch, 0x428, STATIC_DLL_AWORD);

		for (dw = 0 ; dw < 4 ; dw++) {
			/* DWORD master */
			set_dll_val(hdev, base, ch, dw * 0x100 + 0x514,
					STATIC_DLL_ACC);
			/* DWORD slave0 (read rise) */
			set_dll_val(hdev, base, ch, dw * 0x100 + 0x518,
					STATIC_DLL_DWORD_RD_RISE);
			/* DWORD slave1 (read fall) */
			set_dll_val(hdev, base, ch, dw * 0x100 + 0x51C,
					STATIC_DLL_DWORD_RD_FALL);
			/* DWORD slave4 (write) */
			set_dll_val(hdev, base, ch, dw * 0x100 + 0x528,
					STATIC_DLL_DWORD_WR);
		}
...
static void device_id_read(struct hl_device *hdev, int device)
{
	u32 id;
	u32 base, rdata, ignore = 0;

	base = GAUDI_HBM_CFG_BASE + device * GAUDI_HBM_CFG_OFFSET;
	ieee1500_inst(hdev, device, ignore, ieee1500_device_id,
			EXTEST_READ_INST);
	rdata = RREG32(base + GAUDI_HBM_CENTER_CHANNEL * 0x1000 + 0x800);
	id = rdata & 0xFFFF;

	/* Display device ID */
	dev_info(hdev->dev, "Device ID read from HBM %d: 0x%x\n", device, id);
}

static void mr_read(struct hl_device *hdev, int device)
{

	int ch;
	u32 base = GAUDI_HBM_CFG_BASE + device * GAUDI_HBM_CFG_OFFSET;
	u32 rdata0[GAUDI_HBM_CHANNELS];
	u32 rdata1[GAUDI_HBM_CHANNELS];
	u32 rdata2[GAUDI_HBM_CHANNELS];
	u32 rdata3[GAUDI_HBM_CHANNELS];

	dev_info(hdev->dev, "MR dump from HBM %d:\n", device);
	dev_info(hdev->dev, "     MR0  MR1  MR2  MR3  MR4  MR5  MR6  MR7\n");

	for (ch = 0 ; ch < GAUDI_HBM_CHANNELS ; ch++) {
		ieee1500_inst(hdev, device, ch, ieee1500_mode_register_dump_set,
							EXTEST_READ_INST);

		rdata0[ch] = RREG32(base + GAUDI_HBM_CENTER_CHANNEL * 0x1000 +
							ch * 0x100 + 0x800);
		rdata1[ch] = RREG32(base + GAUDI_HBM_CENTER_CHANNEL * 0x1000 +
							ch * 0x100 + 0x804);
		rdata2[ch] = RREG32(base + GAUDI_HBM_CENTER_CHANNEL * 0x1000 +
							ch * 0x100 + 0x808);
		rdata3[ch] = RREG32(base + GAUDI_HBM_CENTER_CHANNEL * 0x1000 +
							ch * 0x100 + 0x80C);

		dev_info(hdev->dev,
			"CH%d: 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x\n",
			ch, (rdata0[ch] >> 0) & 0xFF, (rdata0[ch] >> 8) & 0xFF,
			(rdata0[ch] >> 16) & 0xFF, (rdata0[ch] >> 24) & 0xFF,
			(rdata1[ch] >> 0) & 0xFF, (rdata1[ch] >> 8) & 0xFF,
			(rdata1[ch] >> 16) & 0xFF, (rdata1[ch] >> 24) & 0xFF);
	}

	dev_info(hdev->dev, "     MR8  MR9  MR10 MR11 MR12 MR13 MR14 MR15\n");

	for (ch = 0 ; ch < GAUDI_HBM_CHANNELS ; ch++)
		dev_info(hdev->dev,
			"CH%d: 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x\n",
			ch, (rdata2[ch] >> 0) & 0xFF, (rdata2[ch] >> 8) & 0xFF,
			(rdata2[ch] >> 16) & 0xFF, (rdata2[ch] >> 24) & 0xFF,
			(rdata3[ch] >> 0) & 0xFF, (rdata3[ch] >> 8) & 0xFF,
			(rdata3[ch] >> 16) & 0xFF, (rdata3[ch] >> 24) & 0xFF);
}

#define HBM_DEBUGFS(func) \
static ssize_t gaudi_hbm_debugfs_##func##_read(struct file *f, \
							char __user *buf, \
							size_t count, \
							loff_t *ppos) \
{ \
...
\
	if (*ppos) \
		return 0; \
\
	dev_dbg(hdev->dev, "calling %s for HBM device %d\n", #func, \
			gaudi->hbm_device); \
\
	func(hdev, gaudi->hbm_device); \
\
	return 0; \
} \
\
static const struct file_operations gaudi_hbm_##func##_fops = { \
	.owner = THIS_MODULE, \
	.read = gaudi_hbm_debugfs_##func##_read, \
	.write = NULL \
}

HBM_DEBUGFS(mc_rstn);
HBM_DEBUGFS(phy_init);
HBM_DEBUGFS(init_mc);
HBM_DEBUGFS(internal_loopback_test);
HBM_DEBUGFS(hardware_training);
HBM_DEBUGFS(write_read);
HBM_DEBUGFS(write_read_infinite);
HBM_DEBUGFS(stop_mc_bist);
HBM_DEBUGFS(software_training_by_1500);
HBM_DEBUGFS(software_training_by_bist);
HBM_DEBUGFS(set_static_params);
HBM_DEBUGFS(print_training_result);
HBM_DEBUGFS(config_dram_state_wrap);
HBM_DEBUGFS(config_mlb_state_bypass);
HBM_DEBUGFS(config_mlb_state_dword);
HBM_DEBUGFS(config_mlb_state_initial);
HBM_DEBUGFS(config_mlb_state_aword);
HBM_DEBUGFS(config_mc_state_bist);
HBM_DEBUGFS(config_mc_state_normal);
HBM_DEBUGFS(set_mode_reg_by_mlb_wrap);
HBM_DEBUGFS(temperature_read);
HBM_DEBUGFS(device_id_read);
HBM_DEBUGFS(mr_read);
HBM_DEBUGFS(read_ecc_counters);
HBM_DEBUGFS(read_interrupts);
HBM_DEBUGFS(init_ecc);
HBM_DEBUGFS(dbg_write_read_full);
...
void gaudi_init_pll(struct hl_device *hdev)
{
	...
	if (val) {
		...
		if (!hdev->pldm ||
			(hdev->nic_ports_mask & GAUDI_NIC_MASK_NIC0)) {
	...
	if (!hdev->pldm || (hdev->nic_ports_mask & GAUDI_NIC_MASK_NIC0)) {
