...
static int gaudi2_sim_start(struct hl_simulator_device *edev, int major,
...
static void gaudi2_simulator_create_device(struct work_struct *work)
{
	...
	rc = gaudi2_sim_start(edev, edev->major, edev->id - HLV_SIM_ID_OFFSET,
				&edev->hdev);
	if (rc) {
		dev_err(edev->dev,
			"Failed to create Gaudi2 Simulator device\n");
		/* Set hdev to NULL to prevent call of gaudi2_sim_stop() */
		edev->hdev = NULL;
	}
}
...
#define GAUDI2_SIM_IOCTL_DEF(ioctl, _func) \
	[_IOC_NR(ioctl)] = {.cmd = ioctl, .func = _func}

/** Ioctl table */
static const struct gaudi2_simulator_ioctl_desc gaudi2_simulator_ioctls[] = {
	GAUDI2_SIM_IOCTL_DEF(SIMULATOR_IOCTL_GEN_INT,
			gaudi2_simulator_gen_int_ioctl),
	GAUDI2_SIM_IOCTL_DEF(SIMULATOR_IOCTL_PCI_ACCESS_FROM_DEVICE,
			...
			int minor, struct hl_device **hdev)
{
	...
	if (rc) {
		dev_err(edev->dev,
			"Failed to create real device for GAUDI2 simulator\n");
		return rc;
	}
...
static void gaudi2_sim_stop(struct hl_device *hdev)
{
	hl_device_fini(hdev);
	kfree(hdev);
}
...
static void gaudi2_sim_get_nic_info(struct hl_device *hdev)
{
	...
	if ((hdev->asic_type == ASIC_GAUDI2B_SIM) || (hdev->asic_type == ASIC_GAUDI2B_SIM_ARC)) {
		nic_info->link_mask[0] = cpu_to_le64(0xFFFFFF & ~GAUDI2_HLS2_EXTERN_PORTS_MASK);
