...
static void gaudi2_nic_convert_intr_cause(struct hl_sni_eq_intr_cause *to,
...
int gaudi2_nic_handle_sw_error_event(struct hl_device *hdev, u16 event_type, u8 macro_index,
					struct hl_eq_nic_intr_cause *nic_intr_cause)
{
	...
	if (aux_ops->sw_err_event_handler) {
		gaudi2_nic_convert_intr_cause(&sni_intr_cause, nic_intr_cause);
...
int gaudi2_nic_handle_axi_error_response_event(struct hl_device *hdev, u16 event_type,
...
int gaudi2_nic_set_info(struct hl_device *hdev, bool get_from_fw)
{
	...
	if (get_from_fw) {
		...
		nic->use_fw_serdes_info = hdev->gaudi2_setup_type == GAUDI2_SETUP_TYPE_HLS2;
	} else {
...
static bool gaudi2_nic_can_unset_asid_cfg(struct hl_aux_dev *aux_dev)
{
...
static int gaudi2_nic_reset_mac_tx(struct hl_aux_dev *aux_dev, u32 port)
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);
	struct hl_device *hdev = container_of(nic, struct hl_device, nic);
...
static char *gaudi2_nic_get_event_name(struct hl_aux_dev *aux_dev, u16 event_type)
{
	return gaudi2_irq_map_table[event_type].valid ? gaudi2_irq_map_table[event_type].name :
			"N/A Event";
}

static void gaudi2_nic_set_sni_data(struct hl_device *hdev)
{
	struct gaudi2_device *gaudi2 = hdev->asic_specific;
	...
	/* sni2accel */
	gaudi2_aux_ops->can_unset_asid_cfg = gaudi2_nic_can_unset_asid_cfg;
	gaudi2_aux_ops->reset_mac_tx = gaudi2_nic_reset_mac_tx;
	gaudi2_aux_ops->get_event_name = gaudi2_nic_get_event_name;
}
...
static int gaudi2_nic_send_cpucp_packet(struct hl_device *hdev, u32 port,
...
struct hl_nic_funcs gaudi2_nic_funcs = {
	...
	.set_sni_data = gaudi2_nic_set_sni_data,
