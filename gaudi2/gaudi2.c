...
int gaudi2_set_fixed_properties(struct hl_device *hdev)
{
	...
	/*
	 * If LKD is doing the golden configurations then all DRAM properties must be
	 * determined by the user at this point.
	 * If FW is doing the configurations than we will set the DRAM properties after
	 * we'll get the cpucp info packet that includes the binning info
	 */
	...
	sni_prop->clk = GAUDI2_NIC_CLK_FREQ / USEC_PER_SEC;
...
static int gaudi2_tpc_binning_init_prop(struct hl_device *hdev)
{
	...
	if (hdev->tpc_mask != GAUDI2_TPC_FULL_MASK) {
		if (hdev->tpc_binning) {
			dev_err(hdev->dev, "TPC binning mask 0x%llx applied while not all TPCs are enabled (0x%llx)\n",
...
static int gaudi2_cpucp_info_get(struct hl_device *hdev)
{
	...
	if (!hdev->ignore_fw_nic_info)
		rc = gaudi2_sni_set_info(hdev, true);
...
static void gaudi2_init_arcs(struct hl_device *hdev)
{
	...
	for (i = GAUDI2_QUEUE_ID_PDMA_0_0 ; i < GAUDI2_QUEUE_ID_CPU_PQ ; i += 4) {
		...
		if (gaudi2_is_arc_nic_owned(arc_id) &&
				!(hdev->sni_ports_mask & BIT_ULL(arc_id - CPU_ID_NIC_QMAN_ARC0)))
			continue;
...
static int gaudi2_sni_clear_mem(struct hl_device *hdev)
{
	...
	for (i = 0 ; i < sni_prop->nic_drv_size ; i += sizeof(val)) {
		rc = hdev->asic_funcs->access_dev_mem(hdev, PCI_REGION_DRAM,
			sni_prop->nic_drv_addr + i, &val, DEBUGFS_WRITE64);

		if (rc) {
			dev_err(hdev->dev, "Failed to set nic memory. addr: 0x%llx",
						sni_prop->nic_drv_addr + i);
			return rc;
		}
...
int gaudi2_compute_reset_late_init(struct hl_device *hdev)
{
	...
	gaudi2_sni_compute_reset_late_init(hdev);
...
static int gaudi2_mmu_shared_prepare(struct hl_device *hdev, u32 asid)
{
	...
	for (i = 0 ; i < NIC_NUMBER_OF_MACROS ; i++)
		for (q = 0 ; q < NIC_NUMBER_OF_QM_PER_MACRO ; q++) {
			if (!(hdev->sni_ports_mask & BIT(i * NIC_NUMBER_OF_QM_PER_MACRO + q)))
				continue;
						...
						struct hl_eq_nic_intr_cause *nic_intr_cause)
{
	...
	error_count = gaudi2_sni_handle_sw_error_event(hdev, event_type, macro_index,
							nic_intr_cause);
...
static void gaudi2_check_if_razwi_happened(struct hl_device *hdev)
{
	...
	for (mod_idx = 0 ; mod_idx < NIC_NUMBER_OF_PORTS ; mod_idx++)
		if (hdev->sni_ports_mask & BIT(mod_idx))
				...
				u64 *event_mask)
{
	...
	error_count = gaudi2_sni_handle_axi_error_response_event(hdev, event_type, macro_index,
								nic_intr_cause);
					...
					u64 intr_cause_data, u64 *event_mask)
{
	...
	/* TODO: remove once SW-142252 is resolved */
	gaudi2_print_event(hdev, event_type, true,
	...
	for (i = 0 ; i < GAUDI2_NUM_OF_PCIE_ADDR_DEC_ERR_CAUSE ; i++) {
		...
		/*
		 * TODO: enable once SW-142252 is resolved, meanwhile always check for LBW and HBW
		 * additional info as the indication itself is sometimes missing
		 *
...
void gaudi2_handle_eqe(struct hl_device *hdev, struct hl_eq_entry *eq_entry)
{
	...
	case GAUDI2_EVENT_CPU0_STATUS_NIC0_ENG0 ... GAUDI2_EVENT_CPU11_STATUS_NIC11_ENG1:
		hl_sni_send_status(hdev, event_type - GAUDI2_EVENT_CPU0_STATUS_NIC0_ENG0, 0, 0);
		error_count = GAUDI2_NA_EVENT_CAUSE;
		break;
...
static int gaudi2_sni_init(struct hl_device *hdev)
{
	int rc;
