 ...
 * @use_fw_serdes_info: true if NIC should use serdes values from F/W, false if NIC should use hard
 *                      coded values.
...
struct hl_nic_port_funcs {
	void (*spmu_get_stats_info)(struct hl_device *hdev, u32 port, struct hl_en_stat **stats,
					u32 *n_stats);
	...
	int (*send_cpucp_packet)(struct hl_device *hdev, u32 port, enum cpucp_packet_id packet_id,
					int val);
	void (*fw_nic_status)(struct hl_device *hdev, u32 port);
};
