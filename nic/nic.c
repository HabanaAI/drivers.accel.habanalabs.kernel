...
static void hl_nic_convert_cpucp_status(struct cpucp_nic_status *to,
...
static int __hl_nic_send_cpucp_status(struct hl_device *hdev, u32 port,
					struct hl_sni_cpucp_status *sni_status)
{
	...
	hl_nic_convert_cpucp_status(&status, sni_status);
	...
	kfree(pkt);
out:
	nic_funcs->port_funcs->fw_nic_status(hdev, port);

	return rc;
}

static int hl_nic_send_empty_status(struct hl_device *hdev, int port)
{
	...
	return __hl_nic_send_cpucp_status(hdev, port, &status);
}
...
static void hl_nic_spmu_get_stats_info(struct hl_aux_dev *aux_dev, u32 port,
					struct hl_en_stat **stats, u32 *n_stats)
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);
	struct hl_device *hdev = container_of(nic, struct hl_device, nic);
...
static int hl_nic_spmu_config(struct hl_aux_dev *aux_dev, u32 port, u32 num_event_types,
...
static int hl_nic_spmu_sample(struct hl_aux_dev *aux_dev, u32 port, u32 num_out_data,
				u64 out_data[])
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);
	struct hl_device *hdev = container_of(nic, struct hl_device, nic);
...
static int hl_nic_send_cpucp_status(struct hl_aux_dev *aux_dev, u32 port,
					struct hl_sni_cpucp_status *sni_status)
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);
	struct hl_device *hdev = container_of(nic, struct hl_device, nic);

	return __hl_nic_send_cpucp_status(hdev, port, sni_status);
}

static void hl_nic_device_reset(struct hl_aux_dev *aux_dev)
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);
	struct hl_device *hdev = container_of(nic, struct hl_device, nic);
...
static dma_addr_t hl_nic_asic_dma_map_single(struct hl_aux_dev *aux_dev, void *addr, int len)
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);
	struct hl_device *hdev = container_of(nic, struct hl_device, nic);
...
static void hl_nic_asic_dma_unmap_single(struct hl_aux_dev *aux_dev, dma_addr_t dma_addr, int len)
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);
	struct hl_device *hdev = container_of(nic, struct hl_device, nic);
...
static void *hl_nic_dma_alloc_coherent(struct hl_aux_dev *aux_dev, size_t size,
...
static void hl_nic_dma_free_coherent(struct hl_aux_dev *aux_dev, size_t size, void *cpu_addr,
...
static void *hl_nic_dma_pool_zalloc(struct hl_aux_dev *aux_dev, size_t size, gfp_t mem_flags,
					dma_addr_t *dma_handle)
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);
	struct hl_device *hdev = container_of(nic, struct hl_device, nic);
...
static void hl_nic_dma_pool_free(struct hl_aux_dev *aux_dev, void *vaddr, dma_addr_t dma_addr)
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);
	struct hl_device *hdev = container_of(nic, struct hl_device, nic);
...
static int hl_nic_map_vmalloc_range(struct hl_aux_dev *aux_dev, u64 vmalloc_va, u64 device_va,
					u64 size)
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);

	return hl_map_vmalloc_range(nic->ctx, vmalloc_va, device_va, size);
}

static int hl_nic_unmap_vmalloc_range(struct hl_aux_dev *aux_dev, u64 device_va)
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);

	return hl_unmap_vmalloc_range(nic->ctx, device_va);
}

static u64 hl_nic_reserve_va_block(struct hl_aux_dev *aux_dev, u64 size)
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);
	struct hl_device *hdev = container_of(nic, struct hl_device, nic);

	return hl_reserve_va_block(hdev, nic->ctx, HL_VA_RANGE_TYPE_HOST, size, PAGE_SIZE);
}

static int hl_nic_unreserve_va_block(struct hl_aux_dev *aux_dev, u64 addr, u64 size)
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);
	struct hl_device *hdev = container_of(nic, struct hl_device, nic);

	return hl_unreserve_va_block(hdev, nic->ctx, addr, size);
}

static int hl_nic_get_hw_block_handle(struct hl_aux_dev *aux_dev, u64 address, u64 *handle)
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);
	struct hl_device *hdev = container_of(nic, struct hl_device, nic);
...
static int hl_nic_send_cpucp_packet(struct hl_aux_dev *aux_dev, u32 port, int pkt_id, int val)
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);
	struct hl_device *hdev = container_of(nic, struct hl_device, nic);
	struct hl_nic_port_funcs *port_funcs = hdev->asic_funcs->nic_funcs->port_funcs;
...
static int hl_nic_dma_mmap(struct hl_aux_dev *aux_dev, struct vm_area_struct *vma, void *cpu_addr,
...
static int hl_nic_user_mmap(struct hl_aux_dev *aux_dev, struct vm_area_struct *vma)
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);

	return __hl_mmap(nic->ctx->hpriv, vma);
}

static u8 hl_nic_dram_readb(struct hl_aux_dev *aux_dev, u64 addr)
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);
	struct hl_device *hdev = container_of(nic, struct hl_device, nic);
	u64 val = 0;
...
static u32 hl_nic_dram_readl(struct hl_aux_dev *aux_dev, u64 addr)
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);
	struct hl_device *hdev = container_of(nic, struct hl_device, nic);
	u64 val = 0;
...
static u64 hl_nic_dram_readq(struct hl_aux_dev *aux_dev, u64 addr)
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);
	struct hl_device *hdev = container_of(nic, struct hl_device, nic);
	u64 val = 0;
...
static void hl_nic_dram_writeb(struct hl_aux_dev *aux_dev, u8 val, u64 addr)
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);
	struct hl_device *hdev = container_of(nic, struct hl_device, nic);
	u64 data = val;
...
static void hl_nic_dram_writel(struct hl_aux_dev *aux_dev, u32 val, u64 addr)
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);
	struct hl_device *hdev = container_of(nic, struct hl_device, nic);
	u64 data = val;
...
static u32 hl_nic_rreg(struct hl_aux_dev *aux_dev, u32 reg)
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);
	struct hl_device *hdev = container_of(nic, struct hl_device, nic);
...
static void hl_nic_wreg(struct hl_aux_dev *aux_dev, u32 reg, u32 val)
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);
	struct hl_device *hdev = container_of(nic, struct hl_device, nic);
...
static void hl_nic_set_priv_assertions(struct hl_aux_dev *aux_dev, bool enable)
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);
	struct hl_device *hdev = container_of(nic, struct hl_device, nic);
...
static int hl_nic_get_compute_user_ctx(struct hl_aux_dev *aux_dev, int user_fd)
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);
	struct hl_device *hdev = container_of(nic, struct hl_device, nic);
...
static void hl_nic_put_compute_user_ctx(struct hl_aux_dev *aux_dev)
{
	struct hl_nic *nic = container_of(aux_dev, struct hl_nic, sni_aux_dev);
	struct hl_device *hdev = container_of(nic, struct hl_device, nic);
	struct hl_fpriv *hpriv;
...
static int hl_nic_poll_reg(struct hl_aux_dev *aux_dev, u32 reg, u64 timeout_us,
...
static int hl_nic_poll_mem(struct hl_aux_dev *aux_dev, u32 *addr, u32 *val,
...
static void hl_nic_get_cpucp_info(struct hl_aux_dev *aux_dev,
					struct hl_sni_cpucp_info *hl_sni_cpucp_info)
{
	...
	hl_sni_cpucp_info->serdes_type = le16_to_cpu(cpucp_nic_info->serdes_type);
}
...
static int hl_nic_sni_aux_data_init(struct hl_device *hdev)
{
	struct hl_nic_funcs *nic_funcs = hdev->asic_funcs->nic_funcs;
	...
	aux_data->ports_mask = hdev->nic_ports_mask;
	aux_data->ext_ports_mask = nic->eth_ports_mask;
	aux_data->auto_neg_mask = hdev->nic_auto_neg_mask;
	...
	aux_data->skip_phy_init = hdev->skip_nic_phy_init;
	aux_data->load_phy_fw = hdev->nic_load_fw;
	aux_data->cpucp_fw = !!(hdev->fw_components & FW_TYPE_BOOT_CPU);
	aux_data->supports_coresight = hdev->supports_coresight;
	aux_data->use_fw_serdes_info = nic->use_fw_serdes_info;
	aux_data->cache_line_size = asic_props->cache_line_size;
	aux_data->kernel_asid = HL_KERNEL_ASID_ID;
	aux_data->card_location = nic->card_location;
	aux_data->mmu_enable = hdev->mmu_enable;
	aux_data->lanes_per_port = hdev->nic_lanes_per_port;
	aux_data->mmap_type_flag = HL_MMAP_TYPE_NIC_MEM;
	...
	rc = hl_nic_get_asic_type(hdev, &aux_data->asic_type);
	if (rc) {
	...
	aux_ops->spmu_get_stats_info = hl_nic_spmu_get_stats_info;
	aux_ops->spmu_config = hl_nic_spmu_config;
	aux_ops->spmu_sample = hl_nic_spmu_sample;
	aux_ops->send_cpucp_status = hl_nic_send_cpucp_status;
	aux_ops->device_reset = hl_nic_device_reset;
	aux_ops->asic_dma_map_single = hl_nic_asic_dma_map_single;
	aux_ops->asic_dma_unmap_single = hl_nic_asic_dma_unmap_single;
	aux_ops->dma_alloc_coherent = hl_nic_dma_alloc_coherent;
	aux_ops->dma_free_coherent = hl_nic_dma_free_coherent;
	aux_ops->dma_pool_zalloc = hl_nic_dma_pool_zalloc;
	aux_ops->dma_pool_free = hl_nic_dma_pool_free;
	aux_ops->map_vmalloc_range = hl_nic_map_vmalloc_range;
	aux_ops->unmap_vmalloc_range = hl_nic_unmap_vmalloc_range;
	aux_ops->reserve_va_block = hl_nic_reserve_va_block;
	aux_ops->unreserve_va_block = hl_nic_unreserve_va_block;
	aux_ops->get_hw_block_handle = hl_nic_get_hw_block_handle;
	aux_ops->send_cpucp_packet = hl_nic_send_cpucp_packet;
	aux_ops->dma_mmap = hl_nic_dma_mmap;
	aux_ops->user_mmap = hl_nic_user_mmap;
	aux_ops->dram_readb = hl_nic_dram_readb;
	aux_ops->dram_readl = hl_nic_dram_readl;
	aux_ops->dram_readq = hl_nic_dram_readq;
	aux_ops->dram_writeb = hl_nic_dram_writeb;
	aux_ops->dram_writel = hl_nic_dram_writel;
	aux_ops->rreg = hl_nic_rreg;
	aux_ops->wreg = hl_nic_wreg;
	aux_ops->set_priv_assertions = hl_nic_set_priv_assertions;
	aux_ops->get_compute_user_ctx = hl_nic_get_compute_user_ctx;
	aux_ops->put_compute_user_ctx = hl_nic_put_compute_user_ctx;
	aux_ops->poll_reg = hl_nic_poll_reg;
	aux_ops->poll_mem = hl_nic_poll_mem;
	aux_ops->get_cpucp_info = hl_nic_get_cpucp_info;

	nic_funcs->set_sni_data(hdev);
...
static void hl_nic_sni_aux_data_fini(struct hl_device *hdev)
{
...
static void sni_adev_release(struct device *dev)
{
	...
	nic->is_sni_aux_dev_initialized = false;
}

static int hl_nic_sni_aux_drv_init(struct hl_device *hdev)
{
	struct hl_nic *nic = &hdev->nic;
	...
	rc = hl_nic_sni_aux_data_init(hdev);
	if (rc) {
	...
	nic->is_sni_aux_dev_initialized = true;
...
aux_data_free:
	hl_nic_sni_aux_data_fini(hdev);

	return rc;
}

static void hl_nic_sni_aux_drv_fini(struct hl_device *hdev)
{
	struct hl_nic *nic = &hdev->nic;
	struct auxiliary_device *adev;

	if (!nic->is_sni_aux_dev_initialized)
		return;

	adev = &nic->sni_aux_dev.adev;
...
static int hl_nic_sni_aux_drv_init(struct hl_device *hdev)
{
	...
	rc = hl_sni_probe(&hdev->nic.sni_aux_dev);
	if (rc) {
...
static void hl_nic_sni_aux_drv_fini(struct hl_device *hdev)
{
	...
	hl_sni_remove(&hdev->nic.sni_aux_dev);
	hl_nic_sni_aux_data_fini(hdev);
}
#endif
...
int hl_nic_init(struct hl_device *hdev)
{
	...
	nic->eth_ports_mask = hdev->nic_eth_on_internal ? hdev->nic_ports_mask :
	...
	rc = hl_nic_sni_aux_drv_init(hdev);
	if (rc) {
...
void hl_nic_fini(struct hl_device *hdev)
{
	...
	if (!nic->is_initialized)
		return;

	hl_nic_sni_aux_drv_fini(hdev);

	nic->is_initialized = false;
}
...
int hl_nic_control(struct hl_device *hdev, u32 op, void *input,	void *output, struct hl_ctx *ctx)
{
	...
	if (aux_ops->nic_control)
		return aux_ops->nic_control(aux_dev, op, input, output, ctx->asid);
...
int hl_nic_send_status(struct hl_device *hdev, int port, u8 cmd, u8 period)
{
	...
	if (!nic_funcs->get_hw_cap(hdev)) {
		if (cmd != HL_NIC_STATUS_PERIODIC_STOP)
			return hl_nic_send_empty_status(hdev, port);
		return 0;
	}
...
int hl_nic_mmap(struct hl_device *hdev, u32 asid, struct vm_area_struct *vma)
{
	struct hl_nic_funcs *nic_funcs = hdev->asic_funcs->nic_funcs;
	struct hl_aux_dev *aux_dev = &hdev->nic.sni_aux_dev;
	struct hl_sni_aux_ops *aux_ops;
